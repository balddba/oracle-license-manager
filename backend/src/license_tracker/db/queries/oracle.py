"""Oracle database backend using oracledb asyncio API."""
...
class OracleDatabase(Database):
    """Oracle implementation of the Database query interface."""
    ...
    async def _open(self) -> None:
        """Open an async oracledb connection in thin mode."""
        try:
            ...
            logger.bind(dsn=self._dsn, user=self._user).error(
                "oracle_connect_failed err={}",
                exc,
            )
            raise

    async def _close(self) -> None:
        """Close the async oracledb connection."""
    ...
    def _paginate(self, sql: str) -> str:
        """Append Oracle OFFSET/FETCH clauses.

        Args:
            sql (str): Base SELECT statement.

        Returns:
            str: SQL including :offset and :limit binds.
        """
        return f"{sql.rstrip().rstrip(';')} OFFSET :offset ROWS FETCH NEXT :limit ROWS ONLY"
    ...
    async def _fetchall(
        ...
        try:
            try:
                await cursor.execute(sql, self.prepare_params(params))
            except oracledb.DatabaseError as exc:
                logger.bind(dsn=self._dsn).error("sql_execute_failed err={}", exc)
                raise
    ...
    def _require_conn(self) -> oracledb.AsyncConnection:
        """Return the open connection or raise.

        Returns:
            oracledb.AsyncConnection: Active connection.

        Raises:
            RuntimeError: If the connection is not open.
        """
        if self._conn is None:
            raise RuntimeError("OracleDatabase connection is not open")
        return self._conn

    async def verify_connection(self) -> None:
        """Verify the database connection is active by executing a simple query."""
        await self._fetchone("SELECT 1 FROM dual")
